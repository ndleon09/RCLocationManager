...
- (void)_init;
- (void)_addRegionForMonitoring:(CLRegion *)region;
...
- (void)_addRegionForMonitoring:(CLRegion *)region
{
    ...
    NSAssert([CLLocationManager isMonitoringAvailableForClass:[CLRegion class]] || [CLLocationManager authorizationStatus] == kCLAuthorizationStatusAuthorized, @"RegionMonitoring not available!");
...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
{
    ...
    if(abs(howRecent) > 10.0) {
        NSLog(@"locationManager didUpdateLocations with timestamp %@ which is to old to use", newLocation.timestamp);
        return;
    }
    ...
    if (newLocation.horizontalAccuracy <= manager.desiredAccuracy || _isOnlyOneRefresh == YES) {
        ...
        if (self.locationBlock != nil) {
            self.locationBlock(manager, newLocation);
        }
        
        [[NSNotificationCenter defaultCenter] postNotificationName:RCLocationManagerUserLocationDidChangeNotification
                                                            object:self
                                                          userInfo:([NSDictionary dictionaryWithObject:newLocation forKey: RCLocationManagerNotificationLocationUserInfoKey])];
...
-(void)queryingTimerPassed
{
    ...
    if (self.locationBlock != nil) {
        self.locationBlock(self.userLocationManager, _location);
    }
    
    if ([self.delegate respondsToSelector:@selector(locationManager:didUpdateToLocation:fromLocation:)]) {
        [self.delegate locationManager:self didUpdateToLocation:_location];
    }
}
...
- (void)addCoordinateForMonitoring:(CLLocationCoordinate2D)coordinate
{
    ...
    [self addCoordinateForMonitoring:coordinate withRadius:kDefaultRegionRadiusDistance];
}
...
- (void)addCoordinateForMonitoring:(CLLocationCoordinate2D)coordinate withRadius:(CLLocationDistance)radius
{
    ...
    CLCircularRegion *circularRegion = [[CLCircularRegion alloc] initWithCenter:coordinate radius:radius identifier:[NSString stringWithFormat:@"Region with center (%f, %f) and radius (%f)", coordinate.latitude, coordinate.longitude, radius]];
    
    [self addRegionForMonitoring:circularRegion];
}
...
- (void)addRegionForMonitoring:(CLRegion *)region
{
    ...
    if (![self isMonitoringThisRegion:region]) {
        [self _addRegionForMonitoring:region];
    }
}
